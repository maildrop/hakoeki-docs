...
![WindowsSetting1](figure/WindowsSetting1.png)

左の`VPN`を開き、`VPN 接続を追加する`をクリックします。

![WindowsSetting2](figure/WindowsSetting2.png)
...
`ユーザー名`と`パスワード (オプション)`はステップ2の`/etc/swanctl/conf.d/responder.conf`の編集で記述したユーザー名とパスワードを入力します。
`サインイン情報を保存する`にチェックを入れると、VPN接続時にユーザー名とパスワードの入力を省略することができます。
全て入力し終えたら、最後に`保存`をクリックします。

![WindowsSetting3](figure/WindowsSetting3.png)

次に、`アダプターのオプションを変更する`を開きます。

![WindowsSetting4](figure/WindowsSetting4.png)

この中に先ほど追加したVPN接続があるので、これを右クリックして`プロパティ`を開きます。

![WindowsSetting5](figure/WindowsSetting5.png)

まず、`ネットワーク`タブの中の`インターネット プロトコル バージョン 6(TCP/IPv6)`を選択し、`プロパティ`を開きます。

![WindowsSetting6](figure/WindowsSetting6.png)

`詳細設定(V)...`を開きます。

![WindowsSetting7](figure/WindowsSetting7.png)

`リモート ネットワークでデフォルト ゲートウェイを使う(U)`のチェックを外し、`OK`をクリックします。

![WindowsSetting8](figure/WindowsSetting8.png)

以下の画面に戻り、`インターネット プロトコル バージョン 4(TCP/IPv4)`を選択し、`プロパティ`を開きます。

![WindowsSetting9](figure/WindowsSetting9.png)

`詳細設定(V)...`を開きます。

![WindowsSetting10](figure/WindowsSetting10.png)

`リモート ネットワークでデフォルト ゲートウェイを使う(U)`のチェックを外し、`OK`をクリックします。

![WindowsSetting11](figure/WindowsSetting11.png)

以上で接続の設定が完了しました。
最後に、VPNへ接続できるかを確認します。
タスクバーの`インターネットアクセス`を開くと、先ほど追加したVPN接続が表示されているので、クリックして`接続`をクリックします。

![WindowsSetting12](figure/WindowsSetting12.png)

`接続済み`と表示されれば成功になります。

![WindowsSetting13](figure/WindowsSetting13.png)
