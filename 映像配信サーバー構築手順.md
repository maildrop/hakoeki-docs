...
## ステップ1 必要な情報の取得
ここでは、今後の作業で必要にな情報をあらかじめ整理しておきます。
取得した情報はどこかにメモしておくと良いと思います。

### サーバーのドメイン名とIPv4アドレスとIPv6アドレスの取得
...
ここに記載されている値がそれぞれサーバーの`IPv4アドレス`と`ドメイン名`になるので、メモしておきます。
次に、`PTRレコード`を`IPv6`に切り替え、もう一度`逆引きホスト名`の項目を見ます。
  
![IPv6](figure/IPv6.png)
  
この中に、先ほど取得した`ドメイン名`と同一のホスト名があります。(恐らく1番上にあります)
そのドメイン名に対応しているIPアドレスがサーバーの`IPv6アドレス`になるので、メモしておきます。
...
ただしmacOS では、必ずレスポンダが証明書を送ってくるものとして待機します。このために、macOSから接続を行うと、レスポンダが証明書を送ってこないので認証失敗する事になります。
この設定は、イニシエータからレスポンダに証明書が送られてこなくても、証明書を送信するという設定です。

#### proposals
IPsec で使用する暗号化方法を指定します。基本はdefaultで良いのですが、MS-Windows は modp1024 以下しか受け取りません。
